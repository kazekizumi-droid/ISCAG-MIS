...
<head>
  ...
  <style>
    ...
    .notif-title { font-weight: 700; color: var(--primary-dark); font-size: 0.95rem; margin: 0; }
    .notif-time { font-size: 0.75rem; color: var(--text-muted); white-space: nowrap; }
    .notif-message { font-size: 0.85rem; color: var(--text-color); margin: 0 0 8px 0; line-height: 1.4; }

    .type-system { background: var(--border); }
    .type-system svg { fill: var(--text-main) !important; }
    .type-approve { background: var(--success); }
    .type-reject, .type-alert { background: var(--danger); }
    .type-assign { background: var(--accent); }
    .type-request { background: var(--primary); }
    .type-payment { background: #2e5a8a; }
    .type-user { background: var(--info); }
    .type-update { background: var(--accent); }
    .type-schedule { background: #5a2e7a; }
    .type-staff { background: var(--primary-dark); }
    
    .empty-state { padding: 60px 20px; text-align: center; background: white; border-radius: 12px; border: 1px dashed var(--border); }
    
    .notif-detail-view { background: white; border-radius: 12px; border: 1px solid var(--border); padding: 28px; display: none; }
    
    .btn-back {
      padding: 8px 16px; border-radius: 8px; border: 1px solid var(--border); background: white;
      color: var(--text-muted); cursor: pointer; display: inline-flex; align-items: center;
      gap: 6px; font-weight: 600; font-family: inherit; font-size: 0.85rem; margin-bottom: 20px; transition: all 0.18s;
    }
    .btn-back:hover { background: #f0f2f1; border-color: var(--primary); color: var(--primary); }
    
    .detail-meta { display: flex; gap: 12px; margin-top: 24px; flex-wrap: wrap; }
    .detail-meta-item { padding: 14px 18px; background: #f8f9fa; border-radius: 8px; border: 1px solid var(--border); flex: 1; min-width: 140px; }
    .detail-meta-item .label { font-size: 0.72rem; color: var(--text-muted); text-transform: uppercase; font-weight: 700; letter-spacing: 0.06em; margin-bottom: 4px; }
    .detail-meta-item .value { font-size: 0.9rem; font-weight: 600; color: var(--primary-dark); }
    
    .btn-go-source {
      display: inline-flex; align-items: center; gap: 6px; padding: 10px 22px; border-radius: 8px;
      background: linear-gradient(135deg, var(--primary-dark), var(--primary-light)); color: white;
      border: none; font-weight: 700; font-size: 0.85rem; font-family: inherit; cursor: pointer;
      margin-top: 24px; transition: all 0.18s; text-decoration: none;
      box-shadow: 0 4px 12px rgba(23,107,69,0.25);
    }
    .btn-go-source:hover { box-shadow: 0 6px 20px rgba(23,107,69,0.35); transform: translateY(-1px); }
    .btn-go-source svg { width: 16px; height: 16px; fill: white; }
  </style>
</head>
  ...
  <div class="app-wrapper">
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <a href="admin_notification.php" class="nav-item active" data-tooltip="Notifications">
          <svg viewBox="0 0 24 24" fill="currentColor">
        ...
        <a href="tenant_timeline.php" class="nav-item" data-tooltip="Logs">
          <svg viewBox="0 0 24 24" fill="currentColor">
            <path
              d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9z" />
          </svg>
        ...
        <div class="section-card">
          <div class="section-card-header">
            <h6><svg viewBox="0 0 24 24">
              ...
              <span id="unread-badge" style="font-size:0.72rem;background:var(--danger);color:white;padding:2px 8px;border-radius:12px;font-weight:700;">0 New</span>
          </div>
